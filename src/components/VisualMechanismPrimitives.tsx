...
export function GPCChromatogramSVG({ title = 'Gel Permeation Chromatography (GPC / SEC) Molecular Weight Distribution' }: { title?: string }) {
  return (
    <figure className="my-6 bg-white border border-slate-200 rounded-2xl p-5 shadow-xs font-sans">
      ...
      <svg viewBox="0 0 650 320" className="w-full h-auto">
        <rect width="650" height="320" fill="#FFFFFF" rx="8" />
        <g stroke="#F1F5F9" strokeWidth="1">
          {[50, 100, 150, 200, 250].map(y => <line key={y} x1={70} y1={y} x2={580} y2={y} />)}
          {[140, 220, 300, 380, 460, 540].map(x => <line key={x} x1={x} y1={30} x2={x} y2={270} />)}
        </g>
        ...
        <path d="M 160,270 Q 240,265 280,180 Q 320,60 360,60 Q 400,60 440,180 Q 480,265 540,270" fill="rgba(16, 185, 129, 0.15)" stroke="#059669" strokeWidth="3.5" />
  ...
  return (
    <figure className="my-6 bg-white border border-slate-200 rounded-2xl p-5 shadow-xs font-sans">
      ...
      <svg viewBox="0 0 650 300" className="w-full h-auto">
        ...
        <g stroke="#F1F5F9" strokeWidth="1">
          {[50, 100, 150, 200, 250].map(y => <line key={y} x1={70} y1={y} x2={580} y2={y} />)}
          {[170, 270, 370, 470, 570].map(x => <line key={x} x1={x} y1={30} x2={x} y2={250} />)}
        </g>
        <g stroke="#0F172A" strokeWidth="2">
        ...
        <text x="330" y="288" textAnchor="middle" fontSize="11" fontWeight="700" fill="#475569">FRACTIONAL EXTENT OF REACTION (p) &rarr;</text>
        <text x="25" y="140" textAnchor="middle" fontSize="11" fontWeight="700" fill="#475569" transform="rotate(-90, 25, 140)">DEGREE OF POLYMERIZATION (Xn)</text>
        <path d="M 70,245 Q 320,240 445,215 T 545,50" fill="none" stroke="#2563EB" strokeWidth="3.5" />
        <text x="500" y="40" fontSize="10" fontWeight="bold" fill="#2563EB">Linear Step-Growth (f=2)</text>
        <path d="M 70,245 Q 260,235 340,160 T 365,25" fill="none" stroke="#DC2626" strokeWidth="3.5" strokeDasharray="4,2" />
...
export function FreeRadicalPolymerizationSVG({ title = 'Free-Radical Chain-Growth Polymerization Kinetics & Rate Laws' }: { title?: string }) {
  return (
    <figure className="my-6 bg-white border border-slate-200 rounded-2xl p-5 shadow-xs font-sans">
      <div className="font-mono text-xs font-bold uppercase text-slate-800 tracking-wider mb-3 flex items-center gap-2 border-b border-slate-100 pb-2">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-3 p-4 rounded-xl bg-slate-50 border border-slate-200">
        <div className="p-3.5 bg-white rounded-xl border border-amber-200 shadow-2xs space-y-2">
          ...
          <div className="p-2 rounded bg-amber-50 border border-amber-100 font-mono text-[11px] font-bold text-amber-950">
            Ri = 2 · f · kd · [I]
          </div>
        </div>
        <div className="p-3.5 bg-white rounded-xl border border-blue-200 shadow-2xs space-y-2">
          ...
          <div className="p-2 rounded bg-blue-50 border border-blue-100 font-mono text-[11px] font-bold text-blue-950">
        ...
        <div className="p-3.5 bg-white rounded-xl border border-emerald-200 shadow-2xs space-y-2">
          ...
          <div className="p-2 rounded bg-emerald-50 border border-emerald-100 font-mono text-[11px] font-bold text-emerald-950">
            Xn = kp·[M] / &radic;( f·kd·kt·[I] )
          </div>
        </div>
      </div>
      <figcaption className="mt-3 text-xs font-mono text-slate-500 text-center">
        Classic Vinyl Chain-Growth Kinetics (Odian Principles): Steady-state assumption yields 1st order in monomer and 0.5 order in initiator.
      </figcaption>
    </figure>
  )
}

// ─── 5. Extrusion Screw Zones ────────────────────────────────────────────────
export function ExtrusionScrewZonesSVG({ title = 'Single-Screw Plasticating Extrusion Functional Geometry (L/D = 24:1 - 30:1)' }: { title?: string }) {
  return (
    <figure className="my-6 bg-white border border-slate-200 rounded-2xl p-5 shadow-xs font-sans">
      ...
      <svg viewBox="0 0 700 230" className="w-full h-auto">
      ...
      <figcaption className="mt-3 text-xs font-mono text-slate-500 text-center">
        Extruder Geometry (Rauwendaal Principles): Tapered screw root compresses solid bed to generate intense frictional dissipation and uniform melt pressure.
      </figcaption>
    </figure>
  )
}

// ─── 6. Spherulite Morphology ─────────────────────────────────────────────────
export function SpheruliteMorphologySVG({ title = 'Polymer Crystallization: Spherulitic Super-Structure & Polarized Optics' }: { title?: string }) {
  return (
    <figure className="my-6 bg-white border border-slate-200 rounded-2xl p-5 shadow-xs font-sans">
      ...
      <svg viewBox="0 0 650 290" className="w-full h-auto">
        ...
        <g transform="translate(160, 145)">
          ...
          {[15, 30, 45, 60, 75, 105, 120, 135, 150, 165, 195, 210, 225, 240, 255, 285, 300, 315, 330, 345].map(deg => {
            ...
            return <line key={deg} x1="0" y1="0" x2={Math.cos(rad) * 100} y2={Math.sin(rad) * 100} stroke="#0284C7" strokeWidth="1.5" strokeDasharray="4,2" opacity="0.75" />
          })}
          <path d="M 0,0 L 40,-95 A 105 105 0 0 1 95,-40 Z" fill="rgba(56, 189, 248, 0.4)" />
          <path d="M 0,0 L -95,-40 A 105 105 0 0 1 -40,-95 Z" fill="rgba(56, 189, 248, 0.4)" />
          <path d="M 0,0 L -40,95 A 105 105 0 0 1 -95,40 Z" fill="rgba(56, 189, 248, 0.4)" />
          <path d="M 0,0 L 95,40 A 105 105 0 0 1 40,95 Z" fill="rgba(56, 189, 248, 0.4)" />
        ...
        <g transform="translate(340, 45)">
          ...
          <path d="M 30,80 L 30,140 Q 40,150 50,140 L 50,80 Q 60,70 70,80 L 70,140 Q 80,150 90,140 L 90,80 Q 100,70 110,80 L 110,140 Q 120,150 130,140 L 130,80" fill="none" stroke="#38BDF8" strokeWidth="3.5" />
          <path d="M 130,110 C 160,80 170,150 200,100" fill="none" stroke="#F59E0B" strokeWidth="2" strokeDasharray="2,2" />
...
// ─── 7. DSC Thermogram ────────────────────────────────────────────────────────
export function DSCFormSVG({ title = 'Differential Scanning Calorimetry (DSC) Heat Flow vs Temperature Curve' }: { title?: string }) {
  return (
    <figure className="my-6 bg-white border border-slate-200 rounded-2xl p-5 shadow-xs font-sans">
      <div className="font-mono text-xs font-bold uppercase text-slate-800 tracking-wider mb-3 flex items-center gap-2 border-b border-slate-100 pb-2">
      ...
      <svg viewBox="0 0 650 300" className="w-full h-auto">
        ...
        <text x="330" y="292" textAnchor="middle" fontSize="11" fontWeight="700" fill="#475569">TEMPERATURE (°C) &rarr;</text>
        <text x="25" y="140" textAnchor="middle" fontSize="11" fontWeight="700" fill="#475569" transform="rotate(-90, 25, 140)">HEAT FLOW (mW / mg, ENDO UP)</text>
        
        {/* Tg Step, Tc Exotherm Peak, Tm Endotherm Peak */}
        <path d="M 70,200 L 150,200 Q 170,200 180,225 L 190,225 L 270,225 Q 310,100 340,225 L 420,225 Q 460,285 500,225 L 590,225" fill="none" stroke="#7C3AED" strokeWidth="3" />
        
        {/* Tg Marker */}
        <circle cx="175" cy="212" r="4" fill="#9333EA" />
        <text x="175" y="185" textAnchor="middle" fontSize="10" fontWeight="bold" fill="#9333EA">Tg (Glass Transition)</text>
        <text x="175" y="197" textAnchor="middle" fontSize="8" fill="#6B21A8">&Delta;Cp Step Change</text>

        {/* Tc Exotherm Peak */}
        <circle cx="310" cy="115" r="4" fill="#059669" />
        <text x="310" y="80" textAnchor="middle" fontSize="10" fontWeight="bold" fill="#059669">Tc (Cold Crystallization Peak)</text>
        <text x="310" y="93" textAnchor="middle" fontSize="8" fill="#047857">Exothermic (&Delta;Hc)</text>

        {/* Tm Endotherm Peak */}
        <circle cx="460" cy="270" r="4" fill="#DC2626" />
        <text x="460" y="295" textAnchor="middle" fontSize="10" fontWeight="bold" fill="#DC2626">Tm (Melting Endotherm Peak)</text>
        <text x="460" y="250" textAnchor="middle" fontSize="8" fill="#991B1B">Endothermic (&Delta;Hm)</text>
      </svg>
      <figcaption className="mt-3 text-xs font-mono text-slate-500 text-center">
        DSC Thermal Analysis (ASTM D3418): % Crystallinity = [(&Delta;Hm - &Delta;Hc) / &Delta;Hm100%] &times; 100. Tg marks baseline shift, Tc exothermic crystallization, and Tm endothermic melting.
      </figcaption>
    </figure>
  )
}

// ─── 8. Tensile Stress Strain ────────────────────────────────────────────────
export function TensileStressStrainSVG({ title = 'Polymer Tensile Engineering Stress-Strain Curve (ISO 527 / ASTM D638)' }: { title?: string }) {
  return (
    <figure className="my-6 bg-white border border-slate-200 rounded-2xl p-5 shadow-xs font-sans">
      <div className="font-mono text-xs font-bold uppercase text-slate-800 tracking-wider mb-3 flex items-center gap-2 border-b border-slate-100 pb-2">
      ...
      <svg viewBox="0 0 650 300" className="w-full h-auto">
        ...
        <text x="330" y="292" textAnchor="middle" fontSize="11" fontWeight="700" fill="#475569">ENGINEERING STRAIN (&epsilon; = &Delta;L / L0) &rarr;</text>
        <text x="25" y="140" textAnchor="middle" fontSize="11" fontWeight="700" fill="#475569" transform="rotate(-90, 25, 140)">ENGINEERING STRESS (&sigma; = F / A0, MPa)</text>
        
        {/* Semi-crystalline Ductile Curve: Linear Hookean -> Yield Point -> Neck Propagation -> Strain Hardening -> Break */}
        <path d="M 70,260 L 150,70 Q 180,65 210,95 Q 350,110 440,100 Q 520,70 560,40" fill="none" stroke="#2563EB" strokeWidth="3.5" />
        
        {/* Hookean Linear Slope (Young's Modulus E) */}
        <line x1="70" y1="260" x2="160" y2="45" stroke="#94A3B8" strokeWidth="1.5" strokeDasharray="3,3" />
        <text x="100" y="130" fontSize="10" fontWeight="bold" fill="#2563EB" transform="rotate(-65, 100, 130)">Young's Modulus (E)</text>

        {/* Yield Stress & Strain */}
        <circle cx="170" cy="67" r="4" fill="#DC2626" />
        <text x="170" y="52" textAnchor="middle" fontSize="10" fontWeight="bold" fill="#DC2626">Yield Point (&sigma;y)</text>

        {/* Cold Drawing / Necking Plateau */}
        <text x="310" y="125" textAnchor="middle" fontSize="10" fontWeight="bold" fill="#D97706">Necking &amp; Cold Drawing Plateau</text>

        {/* Strain Hardening & Fracture */}
        <circle cx="560" cy="40" r="5" fill="#059669" />
        <text x="560" y="25" textAnchor="middle" fontSize="10" fontWeight="bold" fill="#059669">Ultimate Fracture (&sigma;b)</text>
      </svg>
      <figcaption className="mt-3 text-xs font-mono text-slate-500 text-center">
        Ductile Polymer Stress-Strain Behavior: Initial elastic region governed by Young's modulus E = &sigma;/&epsilon;, followed by shear yielding, neck formation, cold-drawing strain orientation, and strain hardening fracture.
      </figcaption>
    </figure>
  )
}
