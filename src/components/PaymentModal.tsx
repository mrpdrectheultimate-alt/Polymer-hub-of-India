...
export default function PaymentModal({
  ...
  const whatsappNumber = '919848022338' // Support & verification WhatsApp line
    ...
    try {
      ...
      if (selectedFile && user) {
        try {
          ...
          // Fallback if storage bucket is not configured
      ...
      // In case of network / table restriction, still allow graceful submission feedback
      setIsSubmitted(true)
      onSuccess?.()
    } finally {
      setSubmitting(false)
          ...
          <div className="bg-slate-900 text-white px-6 py-5 flex items-center justify-between border-b border-slate-800">
            <div className="flex items-center gap-2.5">
              ...
              <div>
                ...
                <p className="text-[11px] text-slate-400 font-mono">Scan UPI QR &middot; Upload Screenshot Proof</p>
              </div>
            </div>
            <button
              onClick={onClose}
              ...
              <>
                ...
                <div className="flex items-center justify-between p-4 rounded-2xl bg-blue-50/70 border border-blue-100">
                  ...
                  <div className="text-right">
                    <span className="inline-flex items-center gap-1 text-[11px] font-mono text-emerald-700 bg-emerald-100/80 px-2.5 py-1 rounded-full font-semibold">
                      <ShieldCheck className="w-3.5 h-3.5" /> Instant Manual Queue
                    </span>
                  </div>
                </div>
                ...
                <div className="space-y-3">
                  <div className="flex items-center justify-between">
                    ...
                    <span className="text-[10px] font-mono text-slate-500">GPay &middot; PhonePe &middot; Paytm &middot; BHIM</span>
                  </div>

                  <div className="flex flex-col sm:flex-row items-center gap-4 p-4 rounded-2xl bg-slate-50 border border-slate-200/80">
                    {/* QR Code */}
                    <div className="w-36 h-36 bg-white p-2.5 rounded-xl border border-slate-200 flex flex-col items-center justify-center shrink-0 shadow-xs relative">
                      <svg viewBox="0 0 120 120" className="w-full h-full text-slate-900">
                        {/* Realistic clean QR code grid vector */}
                      ...
                      <div className="absolute inset-0 flex items-center justify-center">
                        <div className="bg-blue-600 text-white font-mono text-[9px] font-bold px-1.5 py-0.5 rounded shadow-xs">
                          ₹{amount}
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="flex-1 w-full space-y-2.5">
                      <div className="space-y-1">
                        ...
                        <div className="flex items-center gap-1.5 bg-white border border-slate-200 rounded-xl p-2">
                          <code className="text-xs font-mono font-bold text-slate-800 flex-1 truncate">{upiId}</code>
                          <button
                            type="button"
                            onClick={handleCopyUpi}
                            className="px-2.5 py-1 rounded-lg bg-slate-100 hover:bg-slate-200 text-slate-700 text-xs font-mono font-semibold transition-colors flex items-center gap-1"
                          >
                ...
                <form onSubmit={handleSubmitProof} className="space-y-4">
                  <h4 className="text-xs font-mono font-bold uppercase tracking-wider text-slate-700 flex items-center gap-1.5">
                    ...
                    Step 2: Submit Payment Screenshot &amp; UTR
                  </h4>
                    ...
                    {previewUrl ? (
                      <div className="flex items-center gap-3">
                        ...
                        <div className="text-left">
                          ...
                          <p className="text-[10px] text-emerald-700 font-mono">{(selectedFile!.size / 1024).toFixed(1)} KB &middot; Click to change</p>
                        </div>
                      </div>
                    ) : (
                      <>
                        ...
                        <div>
                          ...
                          <p className="text-[10px] text-slate-500 font-mono">Tap to select image (PNG, JPG up to 5MB)</p>
                        </div>
                      </>
                    )}
                  </div>
