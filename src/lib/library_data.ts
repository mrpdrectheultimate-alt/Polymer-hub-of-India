...
// Implements 4-Class Legal Classification System (Class A, B, C, D) & Normalized Block Architecture
// Double-escaped KaTeX formulas (\\bar, \\frac, \\int) for zero rendering defects
...
* **Pressure Drop & Viscosity Matching:** Melt viscosity ($\\eta$) dictates hydraulic pump pressure required to drive molten resin through extrusion screws, manifold channels, hot runners, and intricate mould cavities.
* **Shear Dissipation Heating:** High shear rates inside gate orifices generate localized thermal energy ($\\dot{Q} = \\tau \\cdot \\dot{\\gamma}$), risking thermal degradation in temperature-sensitive resins such as PVC, POM, PBT, and PEEK.
...
Where $\\lambda$ is the characteristic relaxation time of the polymer melt and $a$ governs the transition region curvature.
`,
  ch4: `# Chapter 4: Capillary Rheometry & Bagley / Rabinowitsch Corrections

## 1. Capillary Flow Measurements
A capillary rheometer extrudes polymer melt through a tungsten carbide capillary die (radius $R$, length $L$) at controlled piston speeds ($Q$).
...
$$Re = \\frac{\\rho \\cdot v \\cdot d}{\\mu} > 4000 \\quad (\\text{Turbulent Flow Mandate})$$
`,
...
Reversible Addition-Fragmentation Chain Transfer (RAFT) and Atom Transfer Radical Polymerization (ATRP) suppress bimolecular termination, granting narrow polydispersity index ($PDI < 1.1$) and block copolymer synthesis.
`,
